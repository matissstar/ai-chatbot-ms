...
(function($) {
    ...
    // Translation helper
    var i18n = (typeof ai_chatboot_ms_ajax !== 'undefined' && ai_chatboot_ms_ajax.i18n) ? ai_chatboot_ms_ajax.i18n : {};
    ...
    function refreshNonce(cb) {
        $.post(ai_chatboot_ms_ajax.ajax_url, { action: 'ai_chatboot_ms_refresh_nonce' }, function(r) {
            if (r && r.success && r.data && r.data.nonce) ai_chatboot_ms_ajax.nonce = r.data.nonce;
            cb();
        }).fail(function() { cb(); });
    }
    ...
    function sendMessage(message) {
        ...
        function doSend(isRetry) {
            $.ajax({
                ...
                type: 'POST',
                data: {
                    action: 'ai_chatboot_ms_chat',
                    message: message,
                    nonce: ai_chatboot_ms_ajax.nonce,
                    ...
                    if (!response.success && !isRetry && response.data && response.data.text &&
                        ...
                        $.post(ai_chatboot_ms_ajax.ajax_url, { action: 'ai_chatboot_ms_refresh_nonce' }, function(r) {
                            if (r && r.success && r.data && r.data.nonce) {
                                ai_chatboot_ms_ajax.nonce = r.data.nonce;
                            }
                            doSend(true);
                ...
                btn.on('click', function() {
                    ...
                    function doStarterCall(isRetry) {
                        $.ajax({
                            ...
                            type: 'POST',
                            data: {
                                action: 'ai_chatboot_ms_starter_answer',
                                index: idx,
                                nonce: ai_chatboot_ms_ajax.nonce
                            },
                            success: function(response) {
    ...
    function showProductDetails(productId) {
        ...
        $.ajax({
            ...
            type: 'POST',
            data: {
                action: 'ai_chatboot_ms_get_product_details',
    ...
    function showComparison() {
        ...
        $.ajax({
            ...
            type: 'POST',
            data: {
                action: 'ai_chatboot_ms_compare_products',
    ...
    function addToCartAjax(productId, btn) {
        ...
        $.post(ai_chatboot_ms_ajax.ajax_url, {
            action: 'ai_chatboot_ms_add_to_cart',
            nonce: ai_chatboot_ms_ajax.nonce,
            product_id: productId,
            quantity: 1,
            session_id: msaiSessionId
    ...
    function showShippingSelection() {
        ...
        $.post(ai_chatboot_ms_ajax.ajax_url, {
            action: 'ai_chatboot_ms_get_shipping',
            nonce: ai_chatboot_ms_ajax.nonce
        }).done(function(response) {
            loadDiv.remove();
    ...
    function renderShippingButtons(methods, currency) {
        ...
        var skipBtn = $('<button>').addClass('msai-btn msai-btn-shipping msai-btn-shipping-skip')
            ...
            .on('click', function() {
                window.location.href = (ai_chatboot_ms_ajax.checkout_url || '/checkout/');
            });
    ...
    function selectShippingMethod(method, containerDiv) {
        ...
        $.post(ai_chatboot_ms_ajax.ajax_url, {
            action: 'ai_chatboot_ms_set_shipping',
            nonce: ai_chatboot_ms_ajax.nonce,
            method_id: method.id
        }).done(function(response) {
            if (response.success) {
    ...
    $(document).ready(function() {
        ...
        if (typeof ai_chatboot_ms_ajax === 'undefined') {
            return;
        }
